...
enum HtmlTreeBuilderState {
	Initial {
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (isWhitespace(t)) {
				...
				DocumentType doctype = new DocumentType(tb.settings.normalizeTag(d.getName()), d.getPubSysKey(), d.getPublicIdentifier(), d
						.getSystemIdentifier(), tb.getBaseUri());
	...
	BeforeHtml {
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (t.isDirective()) {
				return true; // ignore jsp Directive
			} else if (t.isDoctype()) {
				tb.error(this);
				return false;
			} else if (t.isComment()) {
	...
	BeforeHead {
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (isWhitespace(t)) {
				...
				return InBody.process(t, tb); // does
												// not
												// transition
		...
		boolean process(Token t, HtmlTreeBuilder tb) {
			...
			case StartTag:
				...
				if (name.equals("html")) {
					...
					// doesn't
					// run script, to handle as noscript)
					tb.insert(start);
		...
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (t.isDoctype()) {
				...
				tb.transition(InHead);
					...
					|| t.isComment()
					|| (t.isStartTag()
							&& StringUtil.in(t.asStartTag().normalName(), "basefont", "bgsound", "link", "meta", "noframes", "style"))) {
		...
		boolean process(Token t, HtmlTreeBuilder tb) {
			...
			case Character: {
				...
				if (c.getData().equals(nullString)) {
					...
					tb.error(this);
					return false;
				} else if (tb.framesetOk() && isWhitespace(c)) { // don't
																	// check
																	// if
																	// whitespace
																	// if
			...
			case StartTag:
				...
				if (name.equals("a")) {
					...
					// one.
					// (Newlines at the start of textarea elements are ignored
					// as an
			...
			case EndTag:
				...
				if (StringUtil.inSorted(name, Constants.InBodyEndAdoptionFormatters)) {
					...
					for (int i = 0; i < 8; i++) {
						...
						// formatting element
						// in the list of active formatting elements relative to
						// the
						...
						for (int j = 0; j < 3; j++) {
							...
							if (lastNode == furthestBlock) {
								...
								// immediately after
								// the new node in the list of active formatting
								// elements.
								// not getting how this bookmark both straddles
								// the element above,
								// but is inbetween here...
							}
						...
						for (Node childNode : childNodes) {
							adopter.appendChild(childNode); // append
															// will
															// reparent.
															// thus
															// the
						...
						// formatting
						// elements at the position of the aforementioned
						// bookmark.
		...
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (t.isEndTag() && t.asEndTag().normalName().equals("caption")) {
				...
				if (processed)
					...
					&& StringUtil.in(t.asEndTag().normalName(), "body", "col", "colgroup", "html", "tbody", "td", "tfoot", "th", "thead",
							"tr")) {
				tb.error(this);
				return false;
			} else {
	...
	InTableBody {
		...
		private boolean exitTableBody(Token t, HtmlTreeBuilder tb) {
			...
			tb.clearStackToTableBodyContext();
			tb.processEndTag(tb.currentElement().nodeName()); // tbody,
																// tfoot,
																// thead
			return tb.process(t);
		}
		...
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (t.isEndTag()) {
				...
				if (StringUtil.in(name, "td", "th")) {
					...
					&& StringUtil.in(t.asStartTag().normalName(), "caption", "col", "colgroup", "tbody", "td", "tfoot", "th", "thead",
							"tr")) {
	...
	InSelectInTable {
		boolean process(Token t, HtmlTreeBuilder tb) {
			if (t.isStartTag()
					&& StringUtil.in(t.asStartTag().normalName(), "caption", "table", "tbody", "tfoot", "thead", "tr", "td", "th")) {
				tb.error(this);
				tb.processEndTag("select");
				return tb.process(t);
			} else if (t.isEndTag()
					&& StringUtil.in(t.asEndTag().normalName(), "caption", "table", "tbody", "tfoot", "thead", "tr", "td", "th")) {
				tb.error(this);
